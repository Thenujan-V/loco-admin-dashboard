...
import {
  normalizePickupPersonsResponse,
  useGetPickupPersonsQuery,
  useTogglePickupPersonStatusMutation,
} from 'src/store/pickup-person/pickup-person-api';
...
const getDefaultDocumentRecord = (person: PickupPersonRow): PickupPersonDocuments => ({
  ...person,
  ...(PICKUP_DOCUMENTS[person.id] ?? PICKUP_DOCUMENTS[501]),
});

export default function PickupPersonDetailsView({ id }: Props) {
  const router = useRouter();
  const { enqueueSnackbar } = useSnackbar();
  const { data } = useGetPickupPersonsQuery();
  const [togglePickupPersonStatus] = useTogglePickupPersonStatusMutation();
  ...
  const person = useMemo(() => {
    const apiPerson = normalizePickupPersonsResponse(data).find((item) => String(item.id) === id);
    ...
    if (apiPerson) {
      return {
        ...
        image: `https://api.dicebear.com/7.x/personas/svg?seed=${encodeURIComponent(`${apiPerson.firstname || 'Pickup'}-${apiPerson.id}`)}`,
      } satisfies PickupPersonRow;
    }

    return MOCK_PICKUP_PERSONS.find((item) => String(item.id) === id) ?? MOCK_PICKUP_PERSONS[0];
  }, [data, id]);

  const [documentData, setDocumentData] = useState<PickupPersonDocuments>(() =>
  ...
  useEffect(() => {
    setDocumentData((prev) => {
      ...
      return {
        ...
        vehiclePictureStatus: prev.vehiclePictureStatus,
        vehiclePictureReason: prev.vehiclePictureReason,
        vehicleDocumentStatus: prev.vehicleDocumentStatus,
        vehicleDocumentReason: prev.vehicleDocumentReason,
      };
    });
  }, [person]);

  const orders = useMemo(() => getOrdersByPickupPersonId(person.id), [person.id]);
  ...
  const handleSaveVerifications = () => {
    ...
    enqueueSnackbar('Pickup person verification details saved locally.', { variant: 'success' });
  };
  ...
  const handleConfirmToggle = useCallback(async () => {
    try {
      await togglePickupPersonStatus({ userId: person.id, isActive: pendingActiveValue }).unwrap();
      enqueueSnackbar(
      ...
      enqueueSnackbar(error?.data?.message || error?.message || 'Failed to update pickup person status.', {
        variant: 'error',
      });
    }
  }, [enqueueSnackbar, pendingActiveValue, person.firstname, person.id, person.lastname, togglePickupPersonStatus]);
        ...
        <Stack spacing={3} sx={{ height: '100%' }}>
          <Card sx={{ p: 3 }}>
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary', mt: 1 }}>
              Admin can manually enable or disable this pickup person here.
            </Typography>
              ...
              label={
                <Box>
                  <Typography variant="subtitle2">
                    {person.isActive ? 'Pickup person is active' : 'Pickup person is inactive'}
                  </Typography>
            ...
            {!orders.length && (
              <TableRow>
                <TableCell colSpan={9} align="center" sx={{ py: 5 }}>
                  <Typography variant="subtitle1">No orders found for this pickup person.</Typography>
                </TableCell>
              </TableRow>
            )}
          </TableBody>
  ...
  const renderVerification = (
    <Box>
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, md: 6, lg: 3 }}>
          {renderDocumentCard('Vehicle Picture', documentData.vehiclePicture, 'vehiclePictureStatus', 'vehiclePictureReason')}
        </Grid>
        <Grid size={{ xs: 12, md: 6, lg: 3 }}>
          {renderDocumentCard('Vehicle Document', documentData.vehicleDocument, 'vehicleDocumentStatus', 'vehicleDocumentReason')}
        </Grid>
      </Grid>
