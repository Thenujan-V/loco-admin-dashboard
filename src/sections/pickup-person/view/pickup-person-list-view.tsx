...
export default function PickupPersonListView() {
  ...
  const { data, isLoading, isError } = useGetPickupPersonsQuery();
  ...
  const normalizedPickupPersons = useMemo(() => {
    return normalizePickupPersonsResponse(data).map((person) => ({
      id: person.id,
      ...
      image: `https://api.dicebear.com/7.x/personas/svg?seed=${encodeURIComponent(`${person.firstname || 'Pickup'}-${person.id}`)}`,
    })) as PickupPersonRow[];
  }, [data]);

  useEffect(() => {
    if (normalizedPickupPersons.length) {
      setTableData(normalizedPickupPersons);
    }
  }, [normalizedPickupPersons]);
  ...
  const handleActiveToggleRequest = useCallback((person: PickupPersonRow, checked: boolean) => {
    setConfirmState({
      open: true,
      person,
      nextValue: checked,
  ...
  const handleConfirmToggle = useCallback(async () => {
    ...
    try {
      ...
      await togglePickupPersonStatus({ userId: person.id, isActive: nextValue }).unwrap();
      enqueueSnackbar(
  ...
  }, [confirmState, enqueueSnackbar, handleCloseConfirm, togglePickupPersonStatus]);
        ...
        <Box>
          ...
          {isLoading && (
            <Typography variant="body2" sx={{ color: 'text.secondary', mt: 1 }}>
              Loading pickup persons...
            </Typography>
          )}
          {isError && (
            <Typography variant="body2" sx={{ color: 'error.main', mt: 1 }}>
              Failed to load pickup persons from API. Showing fallback data.
            </Typography>
          )}
        </Box>
      </Box>
        ...
        <DialogContent>
          <Typography variant="body2">
            {confirmState.person
              ...
              : 'Are you sure you want to update this pickup person status?'}
          </Typography>
        </DialogContent>
        <DialogActions>
